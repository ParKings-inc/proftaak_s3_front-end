...
const ShowAvailableSpaceReservationFormStep2 = (props) => {
  ...
  async function ClaimSpot(spaceID, carID, ArrivalTime, DepartureTime) {
    ...
    await postReservation(
      spaceID,
      carID,
      dayjs(ArrivalTime).format("YYYY-MM-DDTHH:mm:00.000Z"),
      dayjs(DepartureTime).format("YYYY-MM-DDTHH:mm:00.000Z")
    );
    // navigate("/reservations");
  }
  if (
    props.AvailableSpaces !== undefined &&
    props.AvailableSpaces != null &&
    Array.isArray(props.AvailableSpaces)
  ) {
    ...
    spaces = (
      <div>
        {" "}
        {props.AvailableSpaces.map((AvailableSpace) => {
          return (
